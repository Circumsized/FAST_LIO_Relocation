...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  ...
  default:
    RCLCPP_ERROR(rclcpp::get_logger("preprocess"), "Error LiDAR Type");
    break;
  }
  *pcl_out = pl_surf;
}
...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg)
{
  ...
  pl_corn.clear();
  pl_full.clear();
  int plsize = std::max(0, std::min<int>(msg->point_num, msg->points.size()));
  ...
  if (feature_enabled)
  {
    ...
    static rclcpp::Clock feature_log_clock(RCL_ROS_TIME);
    RCLCPP_INFO_THROTTLE(rclcpp::get_logger("preprocess"), feature_log_clock,
                         5000, "Feature extraction time: %lf", time / count);
  }
  else
  {
    for(uint i=1; i<(uint)plsize; i++)
...
void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
    ...
    double omega_l = 0.361 * std::max(1, SCAN_RATE);
...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types)
{
  ...
  if(plsize == 0)
  {
    RCLCPP_WARN(rclcpp::get_logger("preprocess"), "something wrong");
    return;
  }
  uint head = 0;
  ...
  for(uint i=head; i<(uint)plsize2; i++)
  {
    ...
    if(plane_type == 1)
    {
      if(i_nex >= types.size()) i_nex = types.size()-1;
  ...
  for(uint i=head+3; i<(uint)plsize2; i++)
  {
    ...
    for(int j=0; j<2; j++)
    {
      ...
      vecs[j] = vecs[j] - vec_a;
      const double vec_a_norm = vec_a.norm();
      const double vec_norm = vecs[j].norm();
      if (vec_a_norm < 1e-12 || vec_norm < 1e-12)
      {
        types[i].edj[j] = Nr_zero;
        continue;
      }

      types[i].angle[j] = vec_a.dot(vecs[j]) / vec_a_norm / vec_norm;
    ...
    const double prev_norm = vecs[Prev].norm();
    const double next_norm = vecs[Next].norm();
    if (prev_norm < 1e-12 || next_norm < 1e-12)
    {
      continue;
    }
    types[i].intersect = vecs[Prev].dot(vecs[Next]) / prev_norm / next_norm;
  ...
  for(uint j=head; j<(uint)plsize; j++)
  {
    if(types[j].ftype==Poss_Plane || types[j].ftype==Real_Plane)
    {
      ...
      if(j == (uint)(last_surf+point_filter_num-1))
      {
        PointType ap;
        ap.x = pl[j].x;
        ap.y = pl[j].y;
    ...
    else
    {
      ...
      if(last_surf != -1)
      {
        ...
        ap.y /= (j-last_surf);
        ap.z /= (j-last_surf);
        ap.intensity /= (j-last_surf);
        ap.curvature /= (j-last_surf);
        pl_surf.push_back(ap);
      }
      last_surf = -1;
    }
...
int Preprocess::plane_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i_cur, uint &i_nex, Eigen::Vector3d &curr_direct)
{
  ...
  {
    if(i_nex >= types.size() || i_nex >= pl.size())
    {
      curr_direct.setZero();
      return 0;
    }
  ...
  if (i_nex >= pl.size() || i_nex >= types.size())
  {
    curr_direct.setZero();
    return 0;
  }
  ...
  for(;;)
  {
    if((i_cur >= pl.size()) || (i_nex >= pl.size()) || (i_nex >= types.size())) break;
  ...
  if(leng_wid < 1e-16 || (two_dis*two_dis/leng_wid) < p2l_ratio)
  {
    curr_direct.setZero();
    return 0;
  }
